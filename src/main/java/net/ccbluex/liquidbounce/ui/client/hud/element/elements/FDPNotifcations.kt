...
import net.ccbluex.liquidbounce.features.module.modules.combat.Aimbot.boolean
import net.ccbluex.liquidbounce.features.module.modules.combat.Aimbot.choices
import net.ccbluex.liquidbounce.features.module.modules.combat.Aimbot.color
import net.ccbluex.liquidbounce.features.module.modules.combat.Aimbot.int
...
import net.ccbluex.liquidbounce.ui.lbppfont.Fonts
import net.ccbluex.liquidbounce.ui.lbppfont.Fonts.fontIconXD85
import net.ccbluex.liquidbounce.ui.lbppfont.Fonts.fontNovoAngularIcon85
import net.ccbluex.liquidbounce.ui.lbppfont.Fonts.fontSFUI35
import net.ccbluex.liquidbounce.ui.lbppfont.Fonts.fontSFUI40
import net.ccbluex.liquidbounce.utils.extras.ColorUtils.potionColor.Companion.getColor
import net.ccbluex.liquidbounce.utils.mobends.util.GUtil.max
import net.ccbluex.liquidbounce.utils.render.ColorUtils.fade
import net.ccbluex.liquidbounce.utils.render.LBPPRenderUtils
import net.ccbluex.liquidbounce.utils.render.LBPPRenderUtils.drawRect
...
import net.ccbluex.liquidbounce.utils.render.shadowRenderUtils.drawShadowWithCustomAlpha
...
@ElementInfo("FDPNotifcations")
class FDPNotifcations(x: Double = 0.0, y: Double = 30.0, scale: Float = 1F,
                      side: Side = Side(Side.Horizontal.RIGHT, Side.Vertical.DOWN)) : Element("FDPNotifcations", x, y, scale, side){
    /**
     * Example notification for CustomHUD designer
     */
    private val exampleNotification = NotificationFDP("Notification", "This is an example notification.", Type.INFO)
    ...
    override fun drawElement(): Border? {
        val notificationsToRemove = mutableListOf<NotificationFDP>()
        for ((index, notificationFDP) in hud.notificationFDP.withIndex()) {
            GL11.glPushMatrix()
            val color1: Int
            val color2: Int
            when (colorMode) {
            ...
            if (notificationFDP.drawNotification(index, styleValue, color1, color2, backgroundMode, backgroundColor)) {
                notificationsToRemove.add(notificationFDP)
            }
            GL11.glPopMatrix()
        }
        notificationsToRemove.forEach { hud.notificationFDP.remove(it) }
        if (mc.currentScreen is GuiHudDesigner) {
            if (!hud.notificationFDP.contains(exampleNotification)) {
                hud.addFDPNotification(exampleNotification)
            }
    ...
    fun drawNotification(index: Int, style: String, colorValue1: Int, colorValue2: Int, backgroundMode: Boolean, backgroundColor: Color): Boolean {
        ...
        if (style == "IDE") {
            ...
            val pn = when (type.name) {
                "SUCCESS" -> ResourceLocation("lizz/notifications/checkmarkIDE.png")
                "ERROR" -> ResourceLocation("lizz/notifications/errorIDE.png")
                "WARNING" -> ResourceLocation("lizz/notifications/warningIDE.png")
                "INFO" -> ResourceLocation("lizz/notifications/infoIDE.png")
                else -> ResourceLocation("lizz/notifications/errorIDE.png")
            }
        ...
        if (style == "ZAVZ") {
            ...
            LBPPRenderUtils.drawGradientSideways(
                1.0,
                height.toFloat() + 0.0,

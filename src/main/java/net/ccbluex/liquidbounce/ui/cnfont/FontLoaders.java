...
public class FontLoaders {
    ...
    public static FontDrawer getFont(String name, int size, boolean antiAliasing) {
        Font font;
        try {
            font = Font.createFont(0, Minecraft.getMinecraft().getResourceManager().getResource(new ResourceLocation("lizz/fonts/" + name)).getInputStream()).deriveFont(Font.PLAIN, (float) size);

 ...
 */
package net.ccbluex.liquidbounce.ui.lbppfont;
...
public class Fonts {
    ...
    @FontDetails(fontName = "Xd Regular", fontSize = 85)
    public static GameFontRenderer fontIconXD85;

    @FontDetails(fontName = "Novogui Regular", fontSize = 85)
    ...
    public static void loadFonts() {
        ...
        ClientUtils.INSTANCE.getLOGGER().info("Loading LBPP Fonts.");
        ...
        ClientUtils.INSTANCE.getLOGGER().info("Loaded LBPP Fonts. (" + (System.currentTimeMillis() - l) + "ms)");
    }
    ...
    private static Font getFont(final String fontName, final int size) {
        try {
            final InputStream inputStream = Minecraft.getMinecraft().getResourceManager().getResource(new ResourceLocation("lizz/lbppfonts/"+fontName)).getInputStream();

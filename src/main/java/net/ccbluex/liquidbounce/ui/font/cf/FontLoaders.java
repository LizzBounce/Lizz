...
public abstract class FontLoaders {
    ...
    public static Font getFont(int size) {
        Font font;
        try {
            font = Font.createFont(0, Minecraft.getMinecraft().getResourceManager().getResource(new ResourceLocation("lizz/fonts/regular.ttf")).getInputStream()).deriveFont(Font.PLAIN, (float) size);
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    private static Font getComfortaa(int size) {
        ...
        try {
            InputStream is = MinecraftInstance.mc.getResourceManager()
                    .getResource(new ResourceLocation("lizz/fonts/Comfortaa.ttf")).getInputStream();
    ...
    private static Font getRoboto(int size) {
        ...
        try {
            InputStream is = MinecraftInstance.mc.getResourceManager()
                    .getResource(new ResourceLocation("lizz/fonts/roboto-regular.ttf")).getInputStream();
            font = Font.createFont(0, is);
            font = font.deriveFont(Font.PLAIN, size);
        } catch (Exception ex) {
            ex.printStackTrace();

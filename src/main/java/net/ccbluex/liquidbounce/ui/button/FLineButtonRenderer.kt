 ...
 */
package net.ccbluex.liquidbounce.ui.button
...
class FLineButtonRenderer(button: GuiButton) : AbstractButtonRenderer(button) {
    ...
    override fun render(mouseX: Int, mouseY: Int, mc: Minecraft) {
        ...
        LBPPRenderUtils.drawRect(button.xPosition.toFloat(), button.yPosition.toFloat(), (button.xPosition + button.width).toFloat(), (button.yPosition + button.height).toFloat(), Color(31, 31, 31, 150).rgb)
        if (button.enabled) {
            val half = button.width / 2.0
            val center = button.xPosition + half
            LBPPRenderUtils.drawRect(center - percent * half, (button.yPosition + button.height - 1).toDouble(), center + percent * half, (button.yPosition + button.height).toDouble(), Color.WHITE.rgb)
        }

        lastUpdate = time

            if (HUD.uiEffectValue && HUD.buttonShadowValue.equals(true)){

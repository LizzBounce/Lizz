...
class BlackoutButtonRenderer(button: GuiButton) : AbstractButtonRenderer(button) {
    ...
    override fun render(mouseX: Int, mouseY: Int, mc: Minecraft) {
        ...
        if (HUD.uiEffectValue && HUD.buttonShadowValue.equals(true)) { shadowRenderUtils.drawShadowWithCustomAlpha(button.xPosition.toFloat(), button.yPosition.toFloat(), button.width.toFloat(), button.height.toFloat(), 240f) }
    }

...
public abstract class MixinMinecraft {
    ...
    @Unique
    private Future<?> lizz$preloadFuture;

    @Unique
    private boolean playXibao_snd = true;

    @Unique
    private boolean isPlayXibao_snd = false;

    @Unique
    private void playXiBao(){


        Thread jlayerThread = new Thread(new Runnable() {
            @Override
            public void run(){
                InputStream inputStream = this.getClass().getResourceAsStream("/assets/minecraft/lizz/sounds/ban.mp3");
    ...
    private void createDisplay(CallbackInfo callbackInfo) {
        if (ClientConfiguration.INSTANCE.getClientTitle()) {
            Display.setTitle(Lizz.INSTANCE.getClientTitle());
        }
    }

    @Inject(method = "displayGuiScreen", at = @At(value = "FIELD", target = "Lnet/minecraft/client/Minecraft;currentScreen:Lnet/minecraft/client/gui/GuiScreen;", shift = At.Shift.AFTER))
    private void hookDisplayGuiScreen(CallbackInfo callbackInfo) {

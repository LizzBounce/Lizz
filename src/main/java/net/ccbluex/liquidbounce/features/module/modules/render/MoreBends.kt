...
object MoreBends : Module("MoreBends", Category.RENDER, forcedDescription = "More Bends") {
    ...
    val texture_NULL: ResourceLocation = ResourceLocation("lizz/mobends/textures/white.png")
    ...
    val onRender3D = handler<Render3DEvent> {
        ...
        while (i < Data_Player.dataList.size) {
            Data_Player.dataList.get(i).update(it.partialTicks)
            ++i
        }
        i = 0
        while (i < Data_Zombie.dataList.size) {
            Data_Zombie.dataList.get(i).update(it.partialTicks)
            ++i
        }
        i = 0
        while (i < Data_Spider.dataList.size) {
            Data_Spider.dataList.get(i).update(it.partialTicks)
            ++i
        }
        if (mc.thePlayer != null) {
            val newTicks: Float =
                mc.thePlayer.ticksExisted.toInt() as Float + it.partialTicks.toInt()

...
object Animations : Module("Animations", Category.RENDER, gameDetecting = false) {
    ...
    val guiAnimations by choices("Container-Animation", arrayOf<String>("None", "Zoom", "Slide", "Smooth"), "Zoom")

    val vSlideValue by choices(
        "Slide-Vertical",
        arrayOf<String>("None", "Upward", "Downward"),
        "Downward")

    val hSlideValue by choices("Slide-Horizontal", arrayOf<String>("None", "Right", "Left"), "Right")

    val animTimeValue by int("Container-AnimTime", 750, 0..3000, "750")
    val tabAnimations by choices("Tab-Animation", arrayOf<String>("None", "Zoom", "Slide"), "Zoom")

    fun getAnimation() = animations.firstOrNull { it.name == animationMode }

    fun getDeltaTime() : Int {
        return RenderUtils.deltaTime
    }

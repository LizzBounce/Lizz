...
object Fucker : Module("Fucker", Category.WORLD) {
    ...
    val onRender3D = handler<Render3DEvent> {
        ...
        drawBlockBox(posToDraw, Color.RED, false)
    }
    ...
    val onRender2D = handler<Render2DEvent> {
        ...
        RenderUtils.drawRoundedRect(barX, barY, barX + barWidth, barY + barHeight, Color(0, 0, 0, 100).rgb, 6F)
        ...
        if (progress > 0F) RenderUtils.drawRoundedGradientRectCorner(
            ...
            barY + barHeight,
            10f,
            Color(173, 216, 230).rgb,
            Color.WHITE.rgb
        )


        val text = "$percentage%"
        val regular35 = Fonts.fontRegular35
        val textWidth = regular35.getStringWidth(text)
        val textX = barX + barWidth - textWidth - 2F
        val textY = barY + (barHeight - regular35.FONT_HEIGHT) / 2F +1f

        regular35.drawString(text, textX, textY, Color.WHITE.rgb, false)
    }

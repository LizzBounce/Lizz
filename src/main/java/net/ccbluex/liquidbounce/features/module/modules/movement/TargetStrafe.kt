...
object TargetStrafe : Module("TargetStrafe", Category.MOVEMENT) {
    ...
    private fun canStrafe(): Boolean {
        ...
        return state && targetEntity != null && (!holdSpaceValue.get() || mc.gameSettings.keyBindJump.isKeyDown) && (!onlySpeedValue.get() || Lizz.moduleManager[Speed::class.java]?.state == true) && (!onlyFlightValue.get() || Lizz.moduleManager.getModule(
            "Fly"
        )!!.state)
    }

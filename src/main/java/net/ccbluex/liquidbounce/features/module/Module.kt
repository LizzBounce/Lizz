            ...
            if (!isStarting) {
                try {
                    if (value) {
                        ...
                        //Sound.INSTANCE.Enable()
                        GBJlayerUtils.playMP3("/assets/minecraft/${CLIENT_NAME.toLowerCamelCase()}/sounds/enable.mp3")
                        WaterMark.showToggleNotification("Module Toggled", "$name has been §2Enabled§f!", true,930,name)
                    }else{
                        //JLayerUtils.playMP3("/assets/minecraft/${CLIENT_NAME.lowercase()}/sounds/Disable/Sigma5.mp3")
                        //Sound.INSTANCE.Disable()
                        GBJlayerUtils.playMP3("/assets/minecraft/${CLIENT_NAME.toLowerCamelCase()}/sounds/disable.mp3")
                        WaterMark.showToggleNotification("Module Toggled", "$name has been §4Disabled§f!", false,930,name)
                    }
                    val state = if (value) {

...
object GBJlayerUtils {
    fun playMP3(resourcePath: String) {
        thread(start = true) {
            try {
                val inputStream: InputStream = Lizz::class.java.getResourceAsStream(resourcePath)
    ...
    fun asyncPlay(resourcePath: String) {
        thread(start = true) {
            try {
                val audioStream = Lizz::class.java.getResourceAsStream(resourcePath)
    ...
    fun getWAVS(resourcePath: String): List<String> {
        ...
        try {
            ...
            val resourceUrl = Lizz::class.java.classLoader.getResource(normalizedPath)
                ?: return emptyList()
